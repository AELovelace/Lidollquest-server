...
export function createDiveControls(db,{now,data,live,current,getFloor,saveFloor,saveCharacter,entry,generate,compute,generator,upgradeFloor}){
 ...
 CREATE INDEX IF NOT EXISTS world_regeneration_active ON world_regeneration(route,status,created);`); // activeJob() runs every route tick; finished jobs keep their candidate floors, so never scan them.
 ...
 const activeJobQuery=db.prepare("SELECT * FROM world_regeneration WHERE route=? AND status IN ('queued','generating','draining') ORDER BY created LIMIT 1"),activeJob=()=>activeJobQuery.get(route); // Prepared once: this runs every second for every route.
 const visitors=()=>db.prepare("SELECT * FROM quest_characters WHERE json_extract(state,'$.dive.route')=?").all(route); // Indexed by quest_character_dive_route instead of parsing every character.
 ...
 const authored=type=>{const def=data.enemies[type];return def&&typeof def.roaming==='boolean'?def.roaming:null;}; // A monster's explicit "Roams by default" trait, or null when its export never said (Quarters, the wilderness routes).
  ...
  if(f.populationRevision!==live.published().revision&&!data.config.full_dungeon_version){ // Full generators own their room population; global publishes must not apply generic top-ups or replace authored bosses.
   ...
   for(const [index,room] of f.rooms.entries()){
    ...
    for(let y=room.y;y<room.y+room.h&&missing>0;y++)for(let x=room.x;x<room.x+room.w&&missing>0;x++)if(walkable(f,x,y)&&![...f.enemies,...f.chests,...(f.pickups??[]),f.entrance,...(f.exits??[]),...visitors().map(c=>JSON.parse(c.state).dive.position)].some(p=>p.x===x&&p.y===y)){
     const type=pick();if(!type)break;const definition=structuredClone(data.enemies[type]),sibling=f.enemies.find(e=>!e.manual&&e.type===type&&typeof e.roaming==='boolean');f.enemies.push({id:'spawn-'+randomUUID(),type,definition,x,y,spawn:{x,y},roaming:authored(type)??sibling?.roaming??enemyRoams(data,{type}),engaged:null,respawnAt:0});missing--;} // A top-up copies an explicit trait, else how its kind already walks this floor, else the generator default.
   }
