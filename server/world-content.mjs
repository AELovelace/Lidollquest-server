...
export function createWorldContent(db,{now=Date.now,spells={},equipment={},defeatEquipment={}}={}){
 ...
 const baselines={monster:new Map(),zone:new Map()},routes=new Map(),compiledSprites=new Set(['sprItem']);let cache=null;
 ...
 function entry(kind,key){const row=db.prepare('SELECT * FROM world_content WHERE kind=? AND id=?').get(kind,key),base=baselines[kind]?.get(key);if(!row&&!base)fail('Content not found.',404);return {kind,id:key,revision:row?.revision??0,draft:row?JSON.parse(row.draft):clone(base),published:row?.published?JSON.parse(row.published):clone(base??null),history:db.prepare('SELECT revision,actor,created FROM world_content_history WHERE kind=? AND id=? ORDER BY revision DESC').all(kind,key)};}
 ...
 function beats(value){
  ...
  const next=v=>v==null||v==='close'||names.has(v)?v:fail('Scene choice points to an unknown page.');
  return value.map((v,i)=>({id:text(v.id??String(i),80),text:text(v.text??''),next:next(v.next),...(v.sprite?{sprite:assetRef(v.sprite)}:{}),...(v.actions?{actions:(Array.isArray(v.actions)&&v.actions.length<=8?v.actions:fail('Use up to eight choices per page.')).map(a=>({label:text(a.label,160),next:next(a.next)}))}:{})}));
 }
 function validate(kind,value){
  ...
  if(kind==='monster'){
   ...
   if(value.defeat){out.defeat={};for(const variant of ['first','repeat']){const scene=value.defeat[variant];if(scene)out.defeat[variant]={title:text(scene.title??'After the battle',100),dialogue:beats(scene.dialogue??[]),aftermath:beats(scene.aftermath??[])};}}
   if(value.defeat_equipment){out.defeat_equipment={};for(const variant of ['first','repeat']){const kit=value.defeat_equipment[variant]??[];if(!Array.isArray(kit)||kit.length>16||kit.some(k=>!Object.hasOwn(equipment,k)))fail('Choose existing defeat equipment.');out.defeat_equipment[variant]=[...kit];}}
   if(Buffer.byteLength(JSON.stringify(out))>12000)fail('Keep monster text and choices below 12 KB.');
   return out;
  }
  if(kind!=='zone'||!routes.has(value.id))fail('Unknown Dive.');
  const out={id:value.id,spawning:!!value.spawning,roaming:!!value.roaming,enemies_per_room:integer(value.enemies_per_room,0,6),enemy_respawn_seconds:integer(value.enemy_respawn_seconds,1,604800),boss_respawn_seconds:integer(value.boss_respawn_seconds,1,604800),pursuit_steps:integer(value.pursuit_steps,0,32),boss_enemy_id:value.boss_enemy_id||null};
 ...
 function resolve(data){ // Preserve route-specific shipped stats until a DM publishes an override for that monster ID.
  ...
  if(t){Object.assign(out.config,{enemies_per_room:t.spawning?t.enemies_per_room:0,enemy_respawn_seconds:t.enemy_respawn_seconds,boss_respawn_seconds:t.boss_respawn_seconds,pursuit_steps:t.pursuit_steps,spawning:t.spawning,roaming:t.roaming});if(t.boss_enemy_id)out.config.boss_enemy_id=t.boss_enemy_id;out.enemy_types=t.pool.map(e=>({...e,chance:e.weight}));}
 ...
 function asset(key){const row=db.prepare('SELECT id,png,frames,width,height FROM world_assets WHERE id=?').get(key??'');if(!row)fail('Artwork not found.',404);return row;}
