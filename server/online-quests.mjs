...
import {fullDungeonQuestPack,fullDungeonNpc,fullDungeonQuestMovement,dungeonDataFor} from './full-dungeon-quests.mjs';
import {campaignChoice,applyDungeonEffects} from './full-dungeon-rules.mjs';
...
export function createOnlineQuests(db,{live,now=Date.now,world,origins,adjust,roll,parties}){
 ...
 function nearby(c,s,key,edition){const p=position(c,s);if(!p||p.seen<=now()-30000)fail('Enter an online zone first.');if(edition&&p.edition!==edition)fail('This map has changed.');const map=placements.view(p.zone),managed=map.placements.find(n=>n.id===key||n.kind==='npc'&&n.content===key),fixture=(map.floor?.fixtures??[]).find(n=>['npc','token'].includes(n.kind)&&(n.id===key||p.zone+':'+n.id===key));const n=managed??fixture;if(!n||Math.abs(n.x-p.x)+Math.abs(n.y-p.y)>1)fail('Stand beside this NPC or objective.');return {p,map,n,key:managed?.content??(fixture.kind==='token'?fixture.content:p.zone+':'+fixture.id)};}
 function npcDefinition(key,n,c,s){const pinned=instances(c).find(q=>['active','choice','ready'].includes(q.state.status)&&q.definition.npcs?.[key])?.definition.npcs[key];return pinned??live.published().npcs[key]??fullDungeonNpc(n,key,s)??{id:key,name:n.name,dialogue:[{id:'greeting',text:n.line??'Hello.',next:'close',actions:[]}],quests:[],sprite:n.sprite??''};}
 ...
 function prepareConversation(c,s,definition){ // Add paged offers without changing the NPC's published dialogue or services.
  ...
  if(definition.campaign_zone){const root=definition.dialogue[0];for(const q of instances(c)){if(q.state.status==='ready'&&q.definition.turn_in.npc===definition.id){const done='_completed_'+q.quest;root.actions.push({label:'Turn in: '+q.definition.name,next:done,effect:'turn_in',quest:q.quest});definition.dialogue.push({id:done,text:q.definition.complete_line??'Thank you for your help.',next:'close',actions:[]});}if(q.state.status==='active')for(const o of q.definition.stages.find(v=>v.id===q.state.stage).objectives)if(o.type==='deliver'&&o.npc===definition.id)root.actions.push({label:'Deliver: '+q.definition.name,next:'close',effect:'deliver',quest:q.quest,objective:o.id});}} // Authored completion prose follows the committed reward receipt.
 ...
 function claim(c,s,key,source){const q=active(c,key);if(!q||q.state.status!=='ready')fail('This quest is not ready to turn in.');if(q.definition.turn_in.mode==='npc'&&source!==q.definition.turn_in.npc)fail('Return to the designated NPC.');if(!s.loadout)fail('Load this character before claiming rewards.');
  ...
  if(r.dignity<0)loseDignity(next.loadout.player_info,-r.dignity,currentTuning());else if(r.dignity>0)next.loadout.player_info.shame=Math.min(1024,dignityOf(next.loadout.player_info)+r.dignity);
  ...
  if(r.xp){next.run={enemy:{exp:r.xp},log:[],hp:next.loadout.player_info.playerHealth,maxHp:next.loadout.player_info.playerHealthMax};awardExperience(next,roll);next.run=null;} // A reward ends its temporary XP context without removing the client's idle-combat field.
 ...
 function act(c,s,input){busy(s);if(input.action==='quest_accept')fail('Speak to the designated NPC and choose a quest in their conversation.');if(input.quest){const q=active(c,input.quest),definition=live.published().quests[input.quest],revision=q?.revision??(definition?digest(definition):null);if(typeof input.quest_revision!=='string'||input.quest_revision!==revision)fail('This quest changed. Refresh before choosing an action.');}if(input.action==='npc_talk'||input.action==='quest_interact'){
   ...
   if(n.kind!=='npc')fail('Choose an NPC.');const definition=clone(npcDefinition(key,n,c,s)),cid=randomUUID(),page=definition.dialogue[0]?.id??'close';definition.id=key;prepareConversation(c,s,definition);
   if(s.loadout.world?.panties_showing)event(c,s,{id:'visible-talk:'+cid,type:'interact',target:'full-visible-conversation',zone:p.zone});
  ...
  if(input.action==='quest_claim')claim(c,s,input.quest,source);
   ...
   let next=choice.next??'close';
   if(choice.effect==='deliver')act(c,s,{...input,action:'quest_deliver',quest:choice.quest,quest_revision:active(c,choice.quest).revision,objective:choice.objective});
   if(talk.definition.campaign_zone){const data=dungeonDataFor(talk.definition.campaign_zone),context={db,data,roll,origins,adjust,now};if(page.give_item)campaignChoice({give_item:page.give_item},c,s,context);next=campaignChoice(choice,c,s,context)??next;
    if(choice.campaign_change){const service=data.npcs[talk.definition.campaign_npc].diaper_change,accepted=roll(100)<service.chance_percent,page=talk.definition.dialogue.find(p=>p.id==='full_change');if(accepted){const pool=service.diaper_pool.filter(id=>!service.always_cursed||data.items[id].cursed),item=pool[roll(pool.length)],old=data.items[s.loadout.player_info.equipped_panties];applyDungeonEffects([{type:'replace_diaper',item}],c,s,context);const text=data.narratives[service.narrative_chunk]?.beats.map(b=>b.text).join('\n\n')??service.success_text??'You receive a fresh {diaper} from {name}.';const words={name:talk.definition.name,npc_name:talk.definition.name,diaper:data.items[item].name,diaper_name:data.items[item].name,zone:data.config.name,old_name:old?.name??'underwear',old_binding:old?.cursed?'The old binding releases.':'The old garment is removed.',new_binding:data.items[item].cursed?'The fresh replacement is cursed.':'The fresh replacement is unbound.'};page.text=text.replace(/\{(\w+)\}/g,(all,key)=>words[key]??all);}else page.text=service.refusal_text;db.prepare('UPDATE online_conversations SET definition=? WHERE character_id=?').run(JSON.stringify(talk.definition),c.id);}
   }
 ...
 function after(c,s,input){if(!db.prepare("SELECT 1 FROM online_quests WHERE character_id=? AND json_extract(state,'$.status') IN ('active','choice') LIMIT 1").get(c.id))return;tickCharacter(c,s);const p=position(c,s);if(p){event(c,s,{id:'zone:'+input.request_id,type:'visit',target:p.zone,zone:p.zone});const map=placements.view(p.zone);fullDungeonQuestMovement(c,s,input,p,map,event);for(const place of map.placements)if(place.kind==='location'&&p.x===place.x&&p.y===place.y)event(c,s,{id:'location:'+input.request_id,type:'visit',target:place.content,zone:p.zone});}}
