...
export function createQuestZones(db,{grant,wallet,adjust,enabled=()=>true,now=Date.now,roll=randomInt}={}) {
 ...
 function settle(i,c,state,run){
  ...
  state.lastResult={outcome:'banked',coins:paid,rounds:run.stage,zone:run.zone};state.wins+=run.stage;state.run=null;
  ...
  return atomic(()=>{
   ...
   if(input.action==='enter'){
    ...
    if(input.action==='appearance'){state.avatar=avatar(input.avatar);} // Cosmetic changes use the same ownership, presence, revision and replay checks as other arena commands.
     ...
     if(!state.run)fail(409,'No active run.');state.lastResult={outcome:'forfeit',coins:0,rounds:state.run.stage-1,zone:z.id};state.run=null;
    }else{
     ...
     if(input.action==='cashout'){if(r.phase!=='interval')fail(409,'Finish the round before banking.');settle(i,c,state,r);}
      ...
      const handicap=roll(3);r.handicaps.push(['Weakened strikes','Reduced vitality','Healing charge lost'][handicap]);
      if(handicap===0)r.attack=Math.max(5,r.attack-1);else if(handicap===1){r.maxHp=Math.max(25,r.maxHp-5);r.hp=Math.min(r.hp,r.maxHp);}else r.heals=Math.max(0,r.heals-1);
      ...
      else{r.enemy.turn++;let hit=z.attack+r.stage+roll(3);if(z.theme==='clockwork'&&r.enemy.turn%3===0)hit+=5;if(z.theme==='mirror')hit+=r.enemy.turn%2===0?5:-2;if(guarded)hit=Math.max(1,Math.floor(hit/3));if(z.theme==='bramble'&&input.action==='attack')hit+=2;r.hp=Math.max(0,r.hp-hit);r.log.push(r.enemy.name+' dealt '+hit+' damage.');
 ...
 return {read,act};
} // Offline stats, inventory, wins and requested award amounts never enter the authoritative simulation.
