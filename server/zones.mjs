...
export function createQuestZones(db,{grant,wallet,adjust,enabled=()=>true,muted=()=>false,now=Date.now,roll=randomInt,measure=(_name,work)=>work(),diveOptions={},desertOptions={},tundraOptions={},taigaOptions={},highDesertOptions={},onPresence=()=>{},compute=null,live=null,audit=()=>{},chatRadius=CHAT_RADIUS_DEFAULT}={}) {
 ...
 const gmTools=createGmTools(db,{now,zone,blocked,isDungeon,dives:engines,quests,live,audit}); // In-game staff commands; the role check lives inside every entry point.
 ...
 function act(secret,input,{buildSnapshot=true}={}){
  ...
  return atomic(()=>{
   ...
   if(input.action==='move'&&divePresence&&!state.dive){const hall=hubRooms.find(r=>r.id===divePresence.zone&&r.kind==='dives'),step={north:[0,-1],south:[0,1],east:[1,0],west:[-1,0]}[input.direction];const gap=hall&&step?hubGaps(hall).find(g=>g.target!==hall.parent&&inHubGap(g,divePresence.x+step[0],divePresence.y+step[1])):null;if(gap){if(live?.published().enabled&&state.contentVersion!==1)fail(409,'Update the game to use published world content.','client_update_required');input={...input,action:'dive_enter',zone:gap.target};}} // Walking into a Dive Hall side opening enters that wilderness route, like the Beds gap enters its annex.
