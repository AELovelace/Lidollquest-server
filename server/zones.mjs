...
export function createQuestZones(db,{grant,wallet,adjust,enabled=()=>true,muted=()=>false,now=Date.now,roll=randomInt,measure=(_name,work)=>work(),diveOptions={},desertOptions={},tundraOptions={},taigaOptions={},highDesertOptions={},hauntedWoodsOptions={},spookyMansionOptions={},autumnalPlainsOptions={},farmsteadOptions={},coastOptions={},calderaOptions={},spaOptions={},onPresence=()=>{},compute=null,live=null,audit=()=>{},chatReach=CHAT_REACH_DEFAULT,followerOptions={},followerChatOptions={}}={}) {
 ...
 function act(secret,input,{buildSnapshot=true}={}){
  ...
  if(!input||!identifier(input.request_id)||!identifier(input.controller))fail(400,'Supply a stable request ID and controller.');
   if(Object.keys(input).some(k=>!['follower_version','npc','full_dungeon_version','puzzle','block','mechanism_revision','quest_version','quest','quest_revision','conversation','placement','choice','branch','objective','content_version','rpp_cost','partners','rp_id','defeat_version','scene','equipment_version','action','request_id','controller','character_id','revision','name','zone','direction','text','channel','avatar','loadout','combat_version','spell','forfeit','stat','edition','encounter','chest','takeover','fixture','offer','slot','bank_item','page','world_step','world_turn_id','steps','item_instance','item_id','creation','online_revision','member','invitation','battle','target','cycle','patch','seq','key','value','mode','kind','amount','index','source','loser','shop','price'].includes(k)))fail(400,'Unsupported zone input.');
  if(input.action==='walk'&&(!Array.isArray(input.steps)||input.steps.length<1||input.steps.length>12||input.steps.some(d=>!Object.hasOwn(FACING,d))||!input.loadout||typeof input.loadout!=='object'))fail(400,'Send one to twelve walking steps with their needs turns.'); // Queued walking: 12 is the hard ceiling of move_burst_steps. // seq: a chat sequence number for gm_chat_delete; key/value: gm_combat_tune; shop/price: companion_roll.
  ...
  return atomic(()=>{
   ...
   if(input.action==='walk'){ // Queued walking: the client ran one needs turn per step (exactly like world_turn) and sends the result with the steps.
    if(state.run||!state.loadout)fail(409,'Finish combat before walking.');
    const before=movementDelay(state.loadout,currentTuning()),next=importLoadout(input.loadout);next.player_info.companions=state.loadout.player_info.companions??{};state.loadout=next;delete state.walkReceipt;
    input={...input,walkDelay:Math.min(before,movementDelay(state.loadout,currentTuning()))}; // Server-only field (after the whitelist): a turn that knocks you to a crawl mid-batch paces from the gentler of the two delays.
   }
   ...
   if(quests&&input.action==='walk'&&divePresence&&quests.placements.rows(divePresence.zone).length){ // Walk the queued path and stop before the first NPC, as a single move would be refused there.
    const npcs=quests.placements.view(divePresence.zone).placements.filter(p=>p.kind==='npc');let x=divePresence.x,y=divePresence.y,cut=0;
    for(const d of input.steps){const [dx,dy]={north:[0,-1],south:[0,1],east:[1,0],west:[-1,0]}[d];x+=dx;y+=dy;if(npcs.some(p=>p.x===x&&p.y===y))break;cut++;}
    if(!cut)fail(409,'An NPC is standing there. Speak to them or walk around.');if(cut<input.steps.length)input={...input,steps:input.steps.slice(0,cut),walkCut:true};
   }
   ...
   if(input.action==='choose_start'){ // Characters made before the religion update pick a starting hub and a free patron, once (client objFaithChoiceUI). Empty zone/key keeps things as they are.
    ...
    if(input.action==='hub_visit'){
     ...
     for(const direction of input.steps){
      const [dx,dy]={north:[0,-1],south:[0,1],east:[1,0],west:[-1,0]}[direction],x=at.x+dx,y=at.y+dy,next=paceStep(clock,now(),input.walkDelay,burst);
      if(next===null){stop='too_fast';break;} // Faster than move_delay_ms on average, even with the burst allowance.
      if(blocked(z,x,y)){stop='blocked';break;}
      if(hubGaps(z).some(g=>inHubGap(g,x,y))||hubPortals(z).some(v=>v.x===x&&v.y===y)||z.parent&&z.exit&&z.exit.x===x&&z.exit.y===y){stop='special';break;} // Travel tiles commit with their own single move and needs turn.
      clock=next;at={x,y};facing=FACING[direction];path.push({x,y});
     }
     if(path.length)db.prepare('UPDATE quest_presence SET x=?,y=?,moved=?,facing=? WHERE owner=?').run(at.x,at.y,clock,facing,i.owner); // One write for the whole batch; the avatar faces its last step.
     state.walkReceipt={request:input.request_id,walked:path.length,stop,path};
   ...
   followers.move(c,state,walk?.path.length>1&&afterPresence?{...afterPresence,...walk.path.at(-2)}:divePresence,afterPresence,afterPresence?(isDungeon(afterPresence.zone)?dive.chatArea({...c,state:JSON.stringify(state)},afterPresence)?.id:afterPresence.zone):null);
   ...
   if(quests)quests.after(c,state,walk?{...input,walkPath:walk.path}:input); // Walks report every tile they crossed, so no location or room is skipped.
   if(walk)delete walk.path; // The client only needs the count and the stop reason.
   ...
   if(state.faith&&state.loadout){ // Piety: each committed step or rest tests the god's uniform; an equipment change only re-reads it (and Sable's changing rooms).
    ...
    if(walk){const lines=[];for(let t=0;t<walk.walked;t++)lines.push(...tickFaith(state,t?state.loadout:prior,state.loadout,{items:withGenerated(hubData.equipment),turn:true,changingRoom:changingRoomAt(afterPresence)}));if(lines.length){state.faithNotice=lines.join(' ');state.faithNoticeAt=now();}} // One piety turn per committed step.
    else if(turn||(['loadout','use_item'].includes(input.action)&&JSON.stringify({...prior,faith:null})!==JSON.stringify({...state.loadout,faith:null}))){
