...
export function createQuestZones(db,{grant,wallet,adjust,enabled=()=>true,muted=()=>false,now=Date.now,roll=randomInt,measure=(_name,work)=>work(),diveOptions={},desertOptions={},tundraOptions={},taigaOptions={},highDesertOptions={},hauntedWoodsOptions={},spookyMansionOptions={},autumnalPlainsOptions={},farmsteadOptions={},coastOptions={},calderaOptions={},spaOptions={},onPresence=()=>{},compute=null,live=null,audit=()=>{},chatRadius=CHAT_RADIUS_DEFAULT}={}) {
 ...
 const hubEvents=live?createHubEncounters(db,{now,roll,parties,live,ids:[...hubCatalog,...hubRooms].map(z=>z.id),definition:id=>{const z=zone(id),d=hubDefinition(z,now(),1,{offers:false});return {...d,width:z.width??20,height:z.height??12,walls:Array.from({length:z.height??12},(_,y)=>Array.from({length:z.width??20},(_,x)=>blocked(z,x,y)?1:0))};}}):null;
 ...
 function snapshotData(i,c=null,view={}){
  ...
  const dungeon=dive.snapshot(c,p),definitions=[...questZones,...hubRooms].map(base=>{
   const z=zone(base.id),definition=hubDefinition(z,now(),shopperLevel(state),{offers:p?.zone===z.id}); // Merchants roll this character's stock at their level, only in the room they stand in; every other room's shelves were rolled and then emptied below.
 ...
 function act(secret,input,{buildSnapshot=true}={}){
  ...
  return atomic(()=>{
   ...
   if(input.action==='enter'){state.contentVersion=input.content_version===1?1:0;state.questVersion=input.quest_version===1?1:0;state.fullDungeonVersion=input.full_dungeon_version===1?1:0;}
