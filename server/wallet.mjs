...
export function createWalletClient({baseUrl,key,fetcher=fetch}={}){
 ...
 async function request(token,body){
  ...
  if(!response.ok&&body?.kind==='debit'&&response.status===409&&data.error_description==='Insufficient balance or account balance limit reached.')throw Object.assign(Error(body.asset==='diamonds'?'Not enough diamonds.':body.asset==='stars'?'Not enough stars.':'Not enough LiDollCoins.'),{status:409,code:'insufficient_balance'});
 ...
 return {
  ...
  async diamonds(token,body){const result=await request(token,body);if(result.request_id!==body.request_id||result.currency!=='Diamonds'||result.kind!==body.kind||result.amount!==1||!Number.isSafeInteger(result.balance)||result.balance<0)throw Error('Invalid diamond receipt');return result;}, // Private sprites cost exactly one diamond; refunds reference that original debit.
