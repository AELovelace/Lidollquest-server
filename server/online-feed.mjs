...
export function createOnlineFeed(db,{token='',now=Date.now}={}){
 ...
 if(!db.prepare('PRAGMA table_info(mommybot_online_events)').all().some(c=>c.name==='kind'))db.exec("ALTER TABLE mommybot_online_events ADD COLUMN kind TEXT NOT NULL DEFAULT 'join'"); // Older rows stay announced as joins.
 db.prepare('INSERT OR IGNORE INTO mommybot_online_meta VALUES (1,?)').run(randomUUID());
 const hash=value=>createHash('sha256').update(value).digest();
 return {
  record(character,presence,arrival=null){
   ...
   if(arrival&&(!previous||presence.seen-previous.seen>120000))db.prepare('INSERT INTO mommybot_online_events(owner,name,joined_at,kind) VALUES (?,?,?,?)').run(character.owner,character.name,now(),arrival);
  ...
  }, // Called inside the authenticated gameplay transaction; only an explicit entry can announce, and brief reconnects still share one arrival.
  route(req,res,url){
   ...
   const events=db.prepare(`SELECT e.id,e.name,e.joined_at,e.kind,EXISTS(SELECT 1 FROM quest_presence p WHERE p.owner=e.owner AND p.seen>?) AS online
  ...
  }, // The server-only credential reveals character names, arrival kinds and join times, never account IDs, grants, inventory or care activity.
