...
export function publishPlayerActivity(db,{previous,state,action,character,owner,area,now}){
 ...
 const zone=area();if(!zone)return; // Companion-only writes and offline imports have no shared area to notify.
 for(const message of events)db.prepare('INSERT INTO quest_chat(zone,owner,character_id,name,text,created) VALUES (?,?,?,?,?,?)').run(zone,'activity:'+owner,character.id,'Activity',message,now());
