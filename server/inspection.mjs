...
export function inspectionProjection(c){
 ...
 return {character_id:c.id,account_id:c.owner,name:c.name,level:Math.max(1,Math.min(1000000,Number(p.level)||1)),class_id:['fighter','mage','diplomat'].includes(p.class_id)?p.class_id:'fighter',revision:c.revision,player_info:out,equipment};
