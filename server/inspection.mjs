...
import {smellOf} from './dignity.mjs';
const catalog=JSON.parse(readFileSync(new URL('./profile-items.json',import.meta.url),'utf8'));
...
export function inspectionProjection(c){
 ...
 out.smell=smellOf(p); // Messes still worn are public: anyone close enough to inspect can smell them.
