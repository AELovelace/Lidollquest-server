...
export function createQuestService({filename=':memory:',walletClient,now=Date.now,roll,log=console.warn,onlineToken=process.env.MOMMYBOT_ONLINE_TOKEN||'',gmToken=process.env.LIDOLLQUEST_GM_TOKEN||''}={}){
 ...
 const server=createServer((req,res)=>{void (async()=>{
  ...
  try{
   ...
   await settlePurchases(verified.owner,token);
   const receipt=result.receipt;identity=verified;try{result=zones.read(token,result.character?.id,req.method==='GET'?view:{companion:['bank_sell','companion_equip','companion_unequip'].includes(input?.action)});if(receipt)result.receipt=receipt;}finally{identity=null;}
   ...
   result.capabilities={unifiedCreation:true,inspection:true,friends:true,cloudSaves:true,saveManagement:true,characterManagement:true,companionEquipment:true,companionBank:true,bankSales:true};
 ...
 server.requestTimeout=10000;server.headersTimeout=5000;server.on('close',()=>{clearInterval(diveTimer);db.close();});return {server,db,gm};
