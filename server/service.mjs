...
export function createQuestService({filename=':memory:',walletClient,now=Date.now,roll,log=console.warn,performanceOptions={},onlineToken=process.env.MOMMYBOT_ONLINE_TOKEN||'',gmAllow=process.env.LIDOLLQUEST_GM_ALLOW||'',gmEnabled=process.env.LIDOLLQUEST_GM_ENABLED!=='false',gmTrustProxy=process.env.LIDOLLQUEST_GM_TRUST_PROXY||'',gmRequireTls=process.env.LIDOLLQUEST_GM_REQUIRE_TLS==='true'}={}){
 ...
 const server=createServer((req,res)=>{void (async()=>{
  ...
  try{
   ...
   let result;identity=verified;try{result=req.method==='GET'?metrics.measure('zones.read',()=>zones.read(token,url.searchParams.get('character_id'),view)):metrics.measure('zones.action',()=>zones.act(token,input));}catch(error){
   ...
   await settlePurchases(verified.owner,token);
   const receipt=result.receipt;identity=verified;try{result=metrics.measure('zones.refresh',()=>zones.read(token,result.character?.id,req.method==='GET'?view:{companion:['bank_sell','companion_equip','companion_unequip'].includes(input?.action)}));if(receipt)result.receipt=receipt;}finally{identity=null;}
 ...
 server.requestTimeout=10000;server.headersTimeout=5000;server.on('close',()=>{clearInterval(diveTimer);metrics.close();db.close();});return {server,db,gm,metrics};
