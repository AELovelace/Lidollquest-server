...
Shared hubs now include Dive Halls with Quarters/Desert floor portals. Gardens default to 40x24 tiles (four times the former area); garden_width/garden_height are exported from online_hubs.json. Definitions include authoritative width/height, spawn, exit and portal style. Garden/Beds/Dive Hall entrances are doors; Shops retains stairs. A dungeon visit keeps both its parent origin hub and returnZone hall; crossing the Desert returns to the other hub hall. Direct lobby dive_enter remains supported for old clients during service-first rollout.
