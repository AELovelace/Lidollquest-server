...
| `gm_god_mode` | optional `value` (true/false; omitted toggles) | Sets `state.godMode` on the GM's own character; snapshots report it as top-level `godMode`. While on, `combat.mjs` `godMode()` makes `enemyAction` land nothing (no hits, enemy spells, debuffs or knockdowns) and any attack, offense/debuff spell, charm or allure defeat the enemy in one hit through the normal win path (XP, loot, quest progress). Heal/cure/buff spells stay ordinary. Never applies in duels (`run.duel`). The flag is dropped on the character's next command once the account loses the `gamemaster` role. Audited as `god_mode`. Covered by `test/god-mode.test.mjs`. |
