...
test('only gamemaster accounts see the flag and can use GM tools, without losing their session',async()=>{
 ...
 try{
  ...
  for(const action of ['gm_catalog','gm_warp_zone','gm_zone_reload']){
   const r=await h.play(playerToken,action,{zone:'princess-rose-shops'});
...
test('the catalogue lists online players and warpable rooms only',async()=>{
 ...
 try{
  ...
  assert.ok(receipt.gm.zones.some(z=>z.id==='princess-rose-shops'));
...
test('warp to zone moves the GM, keeps annex bookkeeping, refuses non-rooms and is audited',async()=>{
 ...
 try{
  ...
  const moved=await h.ok(staffToken,'gm_warp_zone',{zone:'princess-rose-shops'});
  assert.equal(moved.zone,'princess-rose-shops');
  assert.equal(moved.character.hubVisit,'princess-rose-shops'); // Reconnects resume in the annex, exactly as if the GM had walked in.
  const exit=moved.zones.find(z=>z.id==='princess-rose-shops').exit;
