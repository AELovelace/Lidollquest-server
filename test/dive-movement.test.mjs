...
test('server clock pursues and engages on every route, including existing editions without movement flags',()=>{
 ...
 try{
  ...
  for(const data of routes){
   const hub=data.config.hub??(['overworld-desert','overworld-tundra'].includes(data.config.zone_id)?'honeydew-lantern':'princess-rose'),zone=data.config.zone_id??'dive-quarters'; // Lantern's hall still has a Tundra pad; Rose reaches it from its garden wall instead.
   act('enter',{zone:hub,loadout:{player_info:{playerHealth:100,playerHealthMax:100,stat_points:0},inventory:[]}});
   const gate=['overworld-desert','overworld-tundra'].includes(zone); // Every wilderness route opens from a lobby wall now (Honeydew: Tundra west, Desert east).
   if(gate){place(zone==='overworld-tundra'?1:48,25);act('move',{direction:zone==='overworld-tundra'?'west':'east',world_step:true});}
   else{place(...(hub==='honeydew-lantern'?[25,25]:hub==='littlebig-clockwork'?[33,29]:[9,0]));const hall=act('hub_visit',{zone:hub+'-dives'}),pad=hall.zones.find(z=>z.id===hall.zone).portals.find(p=>p.target===zone);
