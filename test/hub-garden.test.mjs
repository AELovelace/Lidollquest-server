...
test('Rose Court is a walled 20x20 garden with the castle gate on its left wall and the Tundra gap on its right wall',()=>{
 ...
 assert.deepEqual(wildernessGates('princess-rose').map(g=>g.target),['dive-tundra']);assert.deepEqual(wildernessGates('honeydew-lantern').map(g=>g.target),['dive-tundra','dive-desert','dive-haunted-woods']); // Honeydew Village's own walls open west onto the Tundra, east onto the Desert and north onto the Haunted Woods.
