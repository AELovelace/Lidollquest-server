...
test('Tundra reset and restart retain committed loot, isolate Quarters fights, and reject stale claims',()=>{
 const f=fixture();try{
  ...
  f.act(a,'dive_enter',{zone:TUNDRA_ZONE});assert.notEqual(f.snap(a).dive.edition,old);assert.equal(f.snap(a).dive.claimed,0);assert.equal(f.snap(a).character.loadout.inventory.filter(i=>i.category!=='ingredient').length,1);
  assert.throws(()=>f.act(a,'dive_claim',{edition:old,chest:chest.id}),/edition changed/);
  f.restart();assert.equal(f.snap(a).character.loadout.inventory.filter(i=>i.category!=='ingredient').length,1);assert.equal(f.db.prepare('SELECT COUNT(*) AS n FROM dive_editions WHERE route=?').get(tundraData.config.route).n,2);
