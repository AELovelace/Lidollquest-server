...
test('all three enlarged markets have clear stairs and reachable services with valid full scenery footprints',()=>{
 for(const z of hubRooms.filter(z=>z.kind==='shops'&&!z.store)){ // Rose Court's Market Hall; LittleBigCity's stores are covered in littlebig-city.test.mjs.
...
test('20-coin curse debit survives a lost reply and service restart with one removal and one charge',async()=>{
 ...
 await start();try{
  await send(body('create',{name:'Cursed'}));await send(body('enter',{zone:'princess-rose-shops',loadout:loadout()}));
