...
test('Honeydew Village is its own 50x50 lobby: gates in both walls, doorsteps into the hall and inn, merchants spread through the clearings',()=>{
 ...
 assert.deepEqual(hubPortals('honeydew-lantern').map(p=>[p.target,p.style,p.x,p.y]),[['dive-tundra','gap',0,24],['dive-desert','gap',49,24],['honeydew-lantern-dives','door',25,24],['honeydew-lantern-beds','door',29,27]]);
 assert.deepEqual(wildernessGates('honeydew-lantern').map(g=>g.target),['dive-tundra','dive-desert']);
...
test('the Community Hall and Inn are the campaign rooms: authored tiles, a door exit, pads in the companion room and beds in the bedrooms',()=>{
 ...
 assert.deepEqual(hall.fixtures,[]);
