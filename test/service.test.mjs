...
test('worker startup prepares every route and HTTP commands/reads each build one settled snapshot',async()=>{
 ...
 try{
  assert.equal(service.db.prepare('SELECT COUNT(*) n FROM dive_editions').get().n,22); // Existing eighteen routes plus four independent full campaign dungeons.
  assert.equal(service.metrics.snapshot().current.workers.workers.reduce((n,w)=>n+w.completed,0),22); // Every full dungeon prepares its edition through the worker pool too.
