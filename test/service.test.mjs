...
test('worker startup prepares every route and HTTP commands/reads each build one settled snapshot',async()=>{
 ...
 try{
  assert.equal(service.db.prepare('SELECT COUNT(*) n FROM dive_editions').get().n,13); // Every route, the Haunted Woods and Spooky Mansion included, prepares one edition.
  assert.equal(service.metrics.snapshot().current.workers.workers.reduce((n,w)=>n+w.completed,0),13); // The Woods' forest and the Mansion's generators run on the worker pool too.
