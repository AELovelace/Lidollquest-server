...
test('walking the city: storefront doorsteps enter and leave stores, purchases work, the plaza doorsteps open the Inn and Coliseum, the Desert gate crosses to Honeydew and back',()=>{
 ...
 try{
  ...
  place(1,30);const desert=act('move',{direction:'west',world_step:true});assert.equal(desert.zone,'overworld-desert');assert.equal(c.dive.gate,true);assert.equal(c.dive.returnZone,'littlebig-clockwork');
