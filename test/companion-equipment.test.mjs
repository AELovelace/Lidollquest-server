...
import {hubData,hubCatalog,hubRooms,hubDefinition,hubArrival} from '../server/hubs.mjs';
...
test('all lobby doors, annex exits and dungeon pads arrive on an adjacent interior tile',()=>{
 for(const root of hubCatalog){
  ...
  for(const portal of lobby.portals){const child=hubRooms.find(r=>r.id===portal.target),arrival=hubArrival(root,child.id),enter=hubArrival(child,root.id);
